...
export default function SpeakerDetailPage() {
  const { id } = useParams(); const { speakers, sessions, speakerMap, now, liveState } = useAppData(); const speaker=speakers.find(s=>s.id===id)
  ...
  const speakerSessions=sessions.filter(s=>(s.speakerIds||[]).includes(id)); const linkedin=normalizeExternalUrl(speaker.linkedinUrl)
